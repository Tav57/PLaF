...
let rec record_lookup : ((string*(bool*exp_val)) list) -> string -> exp_val ea_result =
  fun r id ->
  match r with
  | (_id, (_, v))::_ when _id = id -> return v
  | _::t -> record_lookup t id
  | _ -> error "Id not found in record"

let is_number : exp_val -> exp_val ea_result = 
  fun e ->
  match e with
  | NumVal _ | RefVal _ -> return (BoolVal true)
...
let rec string_of_expval = function
  ...
  | RecordVal(fs) -> "RecordVal("^ String.concat "," (List.map (fun (n,(_,ev)) ->
