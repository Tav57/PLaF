...
let rec find_id : string -> string list -> bool = fun id used_ids ->
  match used_ids with
  | [] -> false
  | h::t ->
    if h = id then true
...
let rec record_get : string -> (string*exp_val) list -> 'a ea_result = fun id r ->
  match r with
  | [] -> error "Proj: field does not exist"
  | (i, n)::t ->
    if i = id then return n
    else record_get id t
